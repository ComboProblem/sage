...
cdef class Bounding_Box_t(object):
    ...
    >>> box_ex
    empty
    >>> box_ex_2 = Bounding_Box_t(box_ex)
    >>> from pplite import Variable, Linear_Expression, Affine_Expression, PPliteGenerator
    >>> x = Variable(0)
    # >>> y = Variable(1)
    # >>> z = Variable(2)
    >>> g = PPliteGenerator('ray', Linear_Expression(x), 0)
    >>> g_1 = PPliteGenerator('ray', -1*Linear_Expression(x), 1)
    # >>> g_2 = PPliteGenerator('point', x+y, 1)
    >>> box_example.add_generators([g, g_1])
    >>> box_example.is_universe()
    False
    >>> box_example
    """
    ...
    # #declator class method to get special instance
    # @classmethod
    # def from_dim_and_spec(cls, sd, se):
    #     pass
    # @classmethod
    # def from_box(cls, other_box):
    #     """
    #     Method for use in initizli

    #     """
    #     if isinstance(other_box, Bounding_Box_t):
    #         y = <Bounding_Box_t> other_box
...
cdef class Bounding_Box_f(object):
    ...
    def __init__(self, box):
        pass
    def __cinit__(self, dim_type sd, Spec_Elem se):
